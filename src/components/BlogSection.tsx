...
export default function BlogSection() {
  const { data: posts, isLoading, error } = useQuery({  // Updated 
query syntax
    queryKey: ['blog-posts'],
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600">Error loading blog posts. Please 
try again later.</p>
      </div>
    );
  }
  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-8">Latest 
Insights</h2>
        <div className="grid md:grid-cols-3 gap-8">
          {posts?.map((post) => (
            <article key={post.id} className="bg-white rounded-xl 
