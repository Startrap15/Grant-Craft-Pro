...
export async function fetchAllBlogPosts() {
    ...
    const posts = [];
    if (wordPressPosts.status === 'fulfilled')
        posts.push(...wordPressPosts.value);
    if (mediumPosts.status === 'fulfilled')
        posts.push(...mediumPosts.value);
    if (ghostPosts.status === 'fulfilled')
        posts.push(...ghostPosts.value);
    if (posts.length === 0) {
